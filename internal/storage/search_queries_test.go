...
func TestItemSearchFiltersAndProductionScope(t *testing.T) {
	...
	lead := makeItem(CreateCostumeItemInput{ProductionID: first.ID, ActorID: ada.ID, ItemTypeID: cloak.ID, Description: "Blue velvet", Status: StatusInProgress, Progress: 40, NextAction: "Fit", Blocker: "Need trim"})
	blocked := makeItem(CreateCostumeItemInput{ProductionID: first.ID, ActorID: bob.ID, ItemTypeID: mask.ID, Description: "Gold mask", Status: StatusBlocked, Progress: 20})
	...
	if err := items.Archive(ctx, first.ID, archivedItem.ID); err != nil { t.Fatal(err) }
	if err := actors.Archive(ctx, first.ID, archived.ID); err != nil { t.Fatal(err) }

	search := NewItemSearch(db)
	all, err := search.Search(ctx, ItemSearchFilter{ProductionID: first.ID})
	if err != nil { t.Fatal(err) }
	if len(all) != 3 || all[0].ID != lead.ID || all[1].ID != blocked.ID || all[2].ID != complete.ID { t.Fatalf("active results = %#v", all) }

	checks := []struct { name string; filter ItemSearchFilter; want int64 }{
		...
		{"type", ItemSearchFilter{ProductionID: first.ID, ItemTypeID: mask.ID}, blocked.ID},
		{"status", ItemSearchFilter{ProductionID: first.ID, Status: StatusBlocked}, blocked.ID},
		{"blocked", ItemSearchFilter{ProductionID: first.ID, Blocked: true}, blocked.ID},
		...
		{"max", ItemSearchFilter{ProductionID: first.ID, MaxProgress: intPtr(25)}, blocked.ID},
		{"text role", ItemSearchFilter{ProductionID: first.ID, Text: "tail"}, blocked.ID},
	}
	for _, check := range checks {
		t.Run(check.name, func(t *testing.T) {
			...
			if check.name == "incomplete" {
				if len(got) != 2 || got[0].ID != lead.ID || got[1].ID != blocked.ID { t.Fatalf("results = %#v, want incomplete active items", got) }
				return
			}
			if check.name == "text role" {
				if len(got) != 2 || got[0].ID != blocked.ID || got[1].ID != complete.ID { t.Fatalf("results = %#v, want Bob's items", got) }
				return
			}
			if len(got) != 1 || got[0].ID != check.want { t.Fatalf("results = %#v, want %d", got, check.want) }
		})
	}
	...
	if err != nil { t.Fatal(err) }
	if len(combined) != 1 || combined[0].ID != lead.ID { t.Fatalf("combined = %#v", combined) }
	otherResults, err := search.Search(ctx, ItemSearchFilter{ProductionID: second.ID})
	if err != nil { t.Fatal(err) }
	if len(otherResults) != 1 || otherResults[0].ID != other.ID { t.Fatalf("second production = %#v", otherResults) }
	archivedResults, err := search.Search(ctx, ItemSearchFilter{ProductionID: first.ID, IncludeArchived: true, Text: "archived"})
	if err != nil { t.Fatal(err) }
	if len(archivedResults) != 2 || archivedResults[0].ID != archivedItem.ID { t.Fatalf("archived = %#v", archivedResults) }
}
...
func TestItemSearchBoundsAndPagination(t *testing.T) {
	...
	items := NewCostumeItemRepository(db)
	for range 7 {
		if _, err := items.Create(ctx, CreateCostumeItemInput{ProductionID: production.ID, ActorID: actor.ID, ItemTypeID: typ.ID}); err != nil { t.Fatal(err) }
	}
	search := NewItemSearch(db)
	first, err := search.Search(ctx, ItemSearchFilter{ProductionID: production.ID, Limit: 3})
	if err != nil { t.Fatal(err) }
	if len(first) != 3 { t.Fatalf("first page length = %d", len(first)) }
	second, err := search.Search(ctx, ItemSearchFilter{ProductionID: production.ID, Limit: 3, Offset: 3})
	if err != nil { t.Fatal(err) }
	if len(second) != 3 || second[0].ID == first[0].ID { t.Fatalf("second page = %#v", second) }
	bounded, err := search.Search(ctx, ItemSearchFilter{ProductionID: production.ID, Limit: 1000})
	if err != nil { t.Fatal(err) }
	if len(bounded) != 7 { t.Fatalf("bounded length = %d", len(bounded)) }
}

func intPtr(value int) *int { return &value }
func stringsToLower(value string) string {
	result := make([]byte, len(value))
	for i := range value { if value[i] >= 'A' && value[i] <= 'Z' { result[i] = value[i] + ('a' - 'A') } else { result[i] = value[i] } }
